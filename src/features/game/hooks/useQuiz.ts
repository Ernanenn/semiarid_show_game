...
import type { Question, Answer, GameSummary } from '../../../shared/types';
...
interface UseQuizReturn {
  ...
  handleAnswerClick: (answer: Answer) => Promise<void>;
...
export function useQuiz(
  ...
  const [questionsDeck, setQuestionsDeck] = useState<Question[]>([]);
  ...
  const finishGame = useCallback(
    ({ totalCorrect = correctAnswers, history = answerHistory }: { totalCorrect?: number; history?: AnswerRecord[] } = {}) => {
