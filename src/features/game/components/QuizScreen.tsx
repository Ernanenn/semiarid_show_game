...
import type { Question, GameSummary } from '../../../shared/types';
...
export function QuizScreen({ questions, onGameOver }: QuizScreenProps): JSX.Element | null {
  const {
    currentQuestion,
    questionNumber,
    totalQuestions,
  ...
  return (
    <QuizContainer role="main" aria-label="Tela do quiz">
      <Header>
        <Question role="heading" aria-level={2}>
          {currentQuestion.question}
        </Question>
        <ProgressText role="status" aria-live="polite">
          Pergunta {questionNumber} de {totalQuestions}
        </ProgressText>
      </Header>

      <AnswersGrid role="group" aria-label="Alternativas de resposta">
        {currentQuestion.answers.map((answer, index) => {
          const status: 'correct' | 'incorrect' | 'neutral' =
          ...
          return (
            <AnswerButton
              ...
              $status={status}
              aria-label={`Alternativa ${letter}: ${answer.text}`}
            >
      ...
      {selectedAnswer?.correct && (
        <NextButton type="button" onClick={handleNextQuestion} aria-label="Próxima pergunta">
