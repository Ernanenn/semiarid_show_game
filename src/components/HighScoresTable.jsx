...
export function HighScoresTable({ limit = 10 }) {
  ...
  const loadScores = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await apiClient.getTopScores(limit);
      setScores(response.scores || []);
    } catch (err) {
      console.error('Erro ao carregar scores:', err);
      // Não mostrar erro se for apenas problema de conexão - apenas não exibir ranking
      setError(null);
      setScores([]);
    } finally {
      setLoading(false);
  ...
  // Não exibir erro - apenas não mostrar ranking se houver problema
  if (error) {
    return null;
  }
